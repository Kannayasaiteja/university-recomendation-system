...
def FilterCollege(user_profile, data):
   ...
   for (k, v) in data.items():
      if user_profile.sat_score >= v.sat and user_profile.max_tuition >= v.tuition and user_profile.sat_score<=1600 and user_profile.sat_score>=400:
          result[k] =v
   return result
...
def main():
   ...
   user_data =extractRankingField(r'D:\csp project\University_Recommendation_System-master\University_Recommendation_System-master\WebScraped_data\csv\merged.csv', ['INSTNM', 'CITY', 'STABBR' ,'TUITIONFEE_OUT',  'SAT_AVG_ALL', 'ADM_RATE_ALL', 'DEBT_MDN_SUPP', 'UGDS_MEN'])
